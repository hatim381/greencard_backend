...
class WithdrawalRequest(db.Model):
    ...
    rib = db.Column(db.String(50), nullable=True)
    status = db.Column(db.String(20), default='en attente')  # en attente, traité, refusé
    requested_at = db.Column(db.DateTime, default=datetime.utcnow)

    user = db.relationship('User', backref='withdrawal_requests')

    def __repr__(self):
        return f'<WithdrawalRequest {self.id} by user {self.user_id}>'
...
class User(db.Model):
    ...
    default_address = db.Column(db.String(255))  # Adresse par défaut de l'utilisateur
